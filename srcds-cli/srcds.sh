...
init() {
	...
	CRASH_DEBUG_MSG="Post about crashes or other issues at https://forums.alliedmods.net/showthread.php?t=158240"
	...
	if [ -d "csgo" ]; then
		DEFAULT_GAME="csgo"
		DEFAULT_MAP="de_dust"
	elif [ -d "insurgency" ]; then
		DEFAULT_GAME="insurgency"
		DEFAULT_MAP="market"
	elif [ -d "doi" ]; then
		DEFAULT_GAME="doi"
		DEFAULT_MAP="bastogne"
	elif [ -d "left4dead" ]; then
		DEFAULT_GAME="left4dead"
		DEFAULT_MAP="l4d_hospital01_apartment"
	elif [ -d "left4dead2" ]; then
		DEFAULT_GAME="left4dead2"
		DEFAULT_MAP="c1m1_hotel"
	elif [ -d "nucleardawn" ]; then
		DEFAULT_GAME="nucleardawn"
		DEFAULT_MAP="downtown"
	elif [ -d "tf" ]; then
		DEFAULT_GAME="tf"
		DEFAULT_MAP="ctf_well"
	elif [ -d "cstrike" ]; then
		DEFAULT_GAME="cstrike"
		DEFAULT_MAP="de_dust"
	elif [ -d "dod" ]; then
		DEFAULT_GAME="dod"
		DEFAULT_MAP="dod_flash"
	elif [ -d "hl2mp" ]; then
		DEFAULT_GAME="hl2mp"
	...
	while test $# -gt 0; do
		case "$1" in
		"-32")
			if test "Darwin" = `uname` -a -d "doi"; then
				PRE_CMD="arch -i386"
			fi ;;
		"-fork")
			NUMFORKS="$2"
...
detectcpu() {
	...
	if test "Linux" = `uname`; then
		...
		HL=./srcds-cli.bundle/Contents/MacOS/srcds-macos
		echo "Using macOS binary."
	else
		echo "Using default binary: $HL"
	fi
}
