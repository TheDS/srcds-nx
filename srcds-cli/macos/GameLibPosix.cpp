...
bool GameLib::Load(const char *name)
{
	...
	if (GameDetector::IsInitialized()) {
		GameDetector &detector = GameDetector::GetInstance();

		// Try game-specific version of library first
		name_ = "game/" + detector.GetGameName() + "/" + name + ".ovrd" LIBEXT;

		if (TryLoad())
			return IsLoaded();

		// Next try engine-specific version
		name_ = AString("engine/") + detector.GetEngineString() + "/" + name + ".ovrd" LIBEXT;
